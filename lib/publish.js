...
module.exports = function (opt, done) {
  ...
  var options = cm._.extend({
    cloneLocation: path.resolve(path.join(process.cwd(), cm.PUBLISH_DIR, cm.env.branch)),
    dirSrc: path.resolve(path.join(process.cwd(), cm.BUILD_DIR ,cm.env.branch)),
    branch: cm.env.branch,
    remote: 'origin',
    message: 'Updates',
  ...
  function e(cmd_tmpl, data) {
    ...
    if (cm.env.verbose) gutil.log('$', gutil.colors.cyan(cmd));
    return sh.exec(cmd);
  }
